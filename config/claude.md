...
- Treat the provenance audit as the necessary first gate, with cryptanalysis
  conditional on what that audit finds — never the reverse.
- Follow the priority order in `research-department.md`: provenance audit
  first, conditional cryptanalysis second, complete public website
  documentation third, and publication of useful intermediate discoveries
  (including a full null result) fourth.
- Prefer questions that can change the route toward a defensible conclusion
  over additional descriptive metrics with no decision attached.
...
- Preserve append-only history, preregistration, sourcing, held-out tests,
...
- Keep the homepage understandable to a typical 10th-grade reader and
  ...
  a cipher has been verified or solved when it has not, and that treats a
  well-documented null result as a genuine win.
- Never treat a TV-show-era (2014–present) claim as equivalent in trust to
  a pre-1900 documented source — the same standing rule
  `agents/historian.md` and `agents/skeptic.md` apply throughout.
