...
Any change to source data, provenance tiering, transcription selection, or a
shared metric must be treated as a dependency-graph change, not as a
one-file correction:
...
When an analysis pins third-party files (source images, transcriptions,
archival scans) by cryptographic checksum, verify the exact repository or
raw-download bytes before parsing. Git may rewrite text line endings on
checkout — especially with `core.autocrlf=true` — so a checksum failure
must be diagnosed before being treated as a source or analysis defect. Do
not silently normalize and accept altered bytes: detect known CRLF-only
conversion, give a targeted remedy (`core.autocrlf=false` or pinned raw
download), and retain the strict expected checksum.
