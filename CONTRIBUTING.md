# Contributing to the Oak Island Collective
...
joins as another contributor — open from launch, unlike the original Voynich
...
reviewable work — most usefully, right now, helping with the primary-source
provenance audit (SQ-1), since that blocks everything else, including any
cryptanalysis.
...
1. **Clone or fork** the repository:
   ```
   git clone https://github.com/SoylentAquamarine/oak-island-collective.git
   ```
...
4. **Pick one bounded task** — do not invent a new primary direction for
   ...
   - Primary-source provenance research (`config/sidequests.md` SQ-1) —
     tracing any specific artifact or claim to its earliest verifiable
     documented source. This is the single most valuable thing a new
   ...
   - **Do not**: edit `knowledge-base/state.md` directly (it only changes
     via PR and review, same rule for everyone), assert that the stone,
     cipher, or decoding is genuine or fabricated without meeting the
     falsification standard, or claim "independently reproduced" without
     actually rerunning the research yourself.
5. **Open a pull request.** The lead agent reviews and merges it, or asks
   ...
   project: sourcing where relevant, disclosed assumptions, and no
...
(SQ-1, primary-source provenance audit, is the current blocking priority)
or a named follow-up in a recent log or comms entry — do not touch
knowledge-base/state.md directly. Do the work, write it up with the same
disclosure standard as the rest of this repo (sources cited, honest null
...
- Contributions that assert a genuine cipher, a fabrication, or a
  decipherment without meeting
  [`methods/falsification-standard.md`](methods/falsification-standard.md).
- Treating any popular retelling (including TV-show claims) as primary-source
  evidence — check recent logs for what tier a claim has actually earned
...
- A candidate decoding built from a handful of resembling symbols without
  first establishing which transcription is most credible and why — the
  specific, well-documented failure mode this project's `agents/skeptic.md`
  and `agents/cryptanalyst.md` exist to catch.
